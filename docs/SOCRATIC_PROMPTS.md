...
3. **Escalating hints** — `hint_level` (0–3) increases when the server detects frustration phrases in the student message (see `chat_utils.py`). At level 3+, the prompt explicitly allows a **more concrete hint** while still blocking verbatim final answers.
